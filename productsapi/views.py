...
import qrcode
from django.contrib.auth.models import User  # Assuming User model for registration
...
def handle_payment_success(request):
    try:
        ...
        if not razorpay_order_id or not razorpay_payment_id:
            logger.error("Missing razorpay_order_id or razorpay_payment_id")
            return Response({"error": "Invalid payment details"}, status=400)

        logger.info(f"Received payment success for order ID: {razorpay_order_id} and payment ID: {razorpay_payment_id}")
        ...
        try:
            ...
            logger.error(f"No booking found for order ID: {razorpay_order_id}")
            return Response({"error": "Booking not found for the given order ID"}, status=404)
        ...
        send_mail(
            ...
            message=f"Your booking with ID {booking.id} is confirmed. Your show name: {booking.movie.name}. Enjoy your show, {booking.user.username}.",
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def download_ticket_api(request, pk):
    booking = get_object_or_404(Booking, pk=pk, user=request.user)
    ticket_url = booking.generate_ticket_url()
...
def generate_pdf(request, pk):
    ...
    print(qr_image_url)  # Debugging: print the QR image URL
